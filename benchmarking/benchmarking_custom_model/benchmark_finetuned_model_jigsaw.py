...
# ╔════════════════════════════════════════════════════════╗
# ║        Custom Dataset class for Jigsaw dataset         ║
# ╚════════════════════════════════════════════════════════╝

# Define a custom Dataset class
class JigsawDataset(Dataset):
    def __init__(self, prompts, msg_ids, labels):
        self.prompts = prompts.tolist() # Convert Series to list
        self.msg_ids = msg_ids.tolist() # Convert Series to list
        self.labels = labels.tolist() # Convert Series to list

    def __len__(self):
        return len(self.prompts)

    def __getitem__(self, idx):
        # Handle cases where an index might be problematic (e.g., if you had filtering logic)
        if idx >= len(self):
            return None # Or raise an IndexError, depending on desired behavior

        return {
            'prompt': self.prompts[idx],
            'msg_id': self.msg_ids[idx],
...
parser.add_argument("--language", type=str, default="fr", help="Language of the jigsaw dataset chosen (fr or en)")
...
content_columns = 'content' if args.language == 'fr' else 'original_text'
...
output_path =DATA_DIR / "benchmark" /  "benchmark_our_custom_model_jigsaw" / f'jigsaw{args.language}_{tuned}output_{dataset_name}_{model_name.split("/")[-1]}{"_" if remove_thoughts else ""}{"-".join(remove_thoughts)}_{rank}.csv'
...
df_jigsaw = pd.read_csv(DATA_DIR / "benchmark_jigsaw" / "benchmark_jigsaw.csv")
...
think_pattern = '<think>\nExplication :\n' if 'a' in dataset_name else ''
df_jigsaw['prompt'] = df_jigsaw[content_columns].apply(lambda x: f"Message:\n{x}\n\nAnalyse:\n{think_pattern}")

# Create an instance of your custom dataset
benchmark_dataset = JigsawDataset(
    prompts=df_jigsaw['prompt'],
    msg_ids=df_jigsaw['msg_id'],
    labels=df_jigsaw['label']
)

sampler = DistributedSampler(benchmark_dataset, num_replicas=world_size, rank=rank, shuffle=False)
loader = DataLoader(
    benchmark_dataset, 
...
all_results = []
for batch_data in tqdm(loader, desc=f"Rank {rank} processing jigsaw split", disable=not is_main_process):
